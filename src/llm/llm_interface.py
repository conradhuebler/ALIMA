...
class LLMInterface(QObject):
    ...
    def _init_watchdog(self):
        ...
        def watchdog_func():
            while self.stream_running:
                ...
                if self.last_chunk_time > 0 and time.time() - self.last_chunk_time > self.request_timeout:
                    self.logger.warning(f"Request timed out after {self.request_timeout} seconds without response")
    ...
    def cancel_generation(self, reason="user_requested"):
        ...
        try:
            if self.current_provider == "openai" and self.current_request_id:
                ...
                self.logger.info(f"No server-side cancellation available for {self.current_provider}")
        except Exception as e:
    ...
    def generate_response(self, provider: str, model: str, prompt: str, temperature: float = 0.7, 
                        seed: Optional[int] = None, image: Optional[Union[str, bytes]] = None, 
                        system: Optional[str] = "", stream: bool = True) -> str:
        """
        ...
        try:
            ...
            if provider == "openai":
                text = self._generate_openai(model, prompt, temperature, seed, image, system, stream)
            elif provider == "gemini":
                text = self._generate_gemini(model, prompt, temperature, seed, image, system, stream)
            elif provider == "anthropic":
                text = self._generate_anthropic(model, prompt, temperature, seed, image, system, stream)
            elif provider == "ollama":
                text = self._generate_ollama(model, prompt, temperature, seed, image, system, stream)
            elif provider == "github":
                text = self._generate_github(model, prompt, temperature, seed, image, system, stream)
            elif provider == "chatai":
                text = self._generate_openai_compatible(model, prompt, temperature, seed, image, system, stream)
            elif provider == "comet":
                text = self._generate_openai_compatible(model, prompt, temperature, seed, image, system, stream)
            elif provider == "azure":
                text = self._generate_azure_inference(model, prompt, temperature, seed, image, system, stream)
            else:
    ...
    def _generate_gemini(self, model: str, prompt: str, temperature: float, seed: Optional[int], 
                    image: Optional[Union[str, bytes]] = None, system: Optional[str] = "", stream: bool = True) -> str:
        ...
        try:
            ...
            if stream:
                try:
                    ...
                    self.logger.warning(f"Streaming fehler: {e}, verwende nicht-streaming Methode")
                    response = model_instance.generate_content(
                        content,
                        generation_config=generation_config,
                        stream=False
    ...
    def _generate_github(self, model: str, prompt: str, temperature: float, seed: Optional[int], 
                    image: Optional[Union[str, bytes]] = None, system: Optional[str] = "", stream: bool = True) -> str:
        ...
        try:
            ...
            if stream:
                try:
                    ...
                    self.logger.warning(f"Erste Streaming-Methode fehlgeschlagen: {e}, versuche Alternative...")
                    ...
                    try:
                        ...
                        self.logger.warning(f"Alle Streaming-Methoden fehlgeschlagen: {stream_error}, verwende non-streaming")
    ...
    def _generate_openai_compatible(self, model: str, prompt: str, temperature: float, seed: Optional[int], 
                                image: Optional[Union[str, bytes]] = None, system: Optional[str] = "", 
        ...
        try:
            ...
            if stream:
                ...
                if hasattr(response_stream, 'id'):
                    ...
                    self.logger.info(f"OpenAI compatible request ID: {self.current_request_id}")
                ...
                for line in response.iter_lines():
                    ...
                    if self.cancel_requested:
                        ...
                        try:
                            ...
                            self.logger.warning(f"Could not cancel Ollama request: {cancel_error}")
