...
class SearchConfig:
    ...
    sort_options: List[str] = field(
        default_factory=lambda: ["Häufigkeit", "Alphabetisch", "Relevanz"]
    )
...
class Config:
    ...
    def __init__(self):
        ...
        # Debug-Ausgabe
        self.logger.debug(
            f"Verfügbare Prompts nach Initialisierung: {self.sections[ConfigSection.PROMPTS].templates.keys()}"
        )
    ...
    def save_config(self):
        ...
        try:
                ...
                for key, value in vars(config).items():
                    ...
                    if not key.startswith("_"):
                        if isinstance(value, AIProvider):
                            ...
                            self.logger.info(
                                f"Verarbeite AI-ProviderKonfiguration: {value}"
                            )
                            ...
                            for (
                                ...
                                self.logger.info(
                                    f"Verarbeite AI-Provider-Konfiguration: {providers_dict}"
                                )
                            value = {
            ...
            self.logger.info(
                f"Konfiguration erfolgreich in {self.config_file} gespeichert"
            )
    ...
    def load_config(self):
        ...
        try:
            if not os.path.exists(self.config_file):
                self.logger.warning(
                    f"Keine Konfigurationsdatei gefunden unter {self.config_file}"
                )
                self._create_default_config()
            ...
            for section_name, section_data in config_data.items():
                ...
                if section == ConfigSection.AI:
                    ...
                    if "templates" in section_data:
                        for name, template_data in section_data["templates"].items():
                            templates[name] = PromptTemplate(
                                ...
                                required_variables=template_data.get(
                                    "required_variables", []
                                ),
                    ...
                    self.logger.warning(
                        f"Unbekannte Konfigurationssektion: {section_name}"
                    )
