# Suggesters — Shared Contract

## [Preserved Section - Permanent Documentation]

- **This package holds only the shared contract** (since July 8, 2026):
  - `BaseSuggester` — abstract contract (Qt-signal shim `currentTerm`, `data_dir`); public API for plugin suggesters.
- ✅ **`MetaSuggester` retired (July 8)** — the multi-provider orchestrator was replaced by `src/core/search/service.py` (`search_gnd_keywords` / `resolve_gnd_instances`), which builds providers via `factory.build_provider` from `PluginInstanceConfig`. Classic (`SearchCLI`), MCP (`ToolRegistry`), and GUI (`find_keywords`) all route through it.
