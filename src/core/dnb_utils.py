...
def get_dnb_classification(gnd_id):
    ...
    try:
        ...
        try:
            ...
            result.update(
                {"status": "error", "error_message": f"Netzwerkfehler: {str(e)}"}
            )
            return result
        ...
        try:
            ...
            result.update(
                {"status": "error", "error_message": f"RDF-Parse-Fehler: {str(e)}"}
            )
            return result
        ...
        for type_triple in g.triples((entity_uri, RDF.type, None)):
            ...
            if "standards/elementset/gnd#" in type_uri:
                ...
                if "DifferentiatedPerson" in type_name or "Person" in type_name:
                    ...
                    keyword in type_name
                    for keyword in ["SubjectHeading", "Heading", "Nomenclature"]
                ):
        ...
        for pred in [
            GNDO.relatedDdcWithDegreeOfDeterminacy1,
            ...
            for ddc_triple in g.triples((entity_uri, pred, None)):
                ...
                result["ddc"].append({"code": ddc_code, "determinancy": determinacy})
