...
class CatalogFallbackSuggester(BaseSuggester):
    ...
    def extract_subjects_from_record(self, record_id, record_url, title):
        ...
        """
        if self.debug:
            self.logger.debug(f"Extracting information from record: {record_id} - {title}")
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.warning(f"HTTP {response.status_code} for record {record_id}: {record_url}")
                return result
                ...
                if match:
                    ...
                    if '+' in subject:
                        for s in subject.split('+'):
                            s = s.strip()
                            if s and s not in result['subjects']:  # Ignore empty strings
    ...
    def search_catalog(self, search_term, max_records=None):
        ...
        while processed_records < max_records:
            ...
            try:
                ...
                if response.status_code != 200:
                    self.logger.error(f"HTTP {response.status_code} for search result page {page}: {search_url}")
                    break
                ...
                for record_id, record_url, title in record_links:
                    ...
                    record_data = self.extract_subjects_from_record(record_id, record_url, title)
                    ...
                    if processed_records % 5 == 0 or processed_records == max_records:
                        self.currentTerm.emit(f"{search_term} ({processed_records}/{max_records})")
                ...
                self.logger.error(f"Error processing search result page {page}: {str(e)}")
                break
        ...
        if self.debug:
            self.logger.info(f"Extraction completed: {len(results)} subjects from {processed_records} records")
