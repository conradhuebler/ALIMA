...
- ✅ **`BibRecord` (`src/core/bib_record.py`, WP-D1, July 20)** — one bibliographic record shape; `to_bibrecord(record, source)` normalizes finc/catalog/sru/k10plus. **P1 + P2 landed (Aug 3):** `to_analysis_text()` + `isbn`/`ppn` input sources; record classifications flow as dk_classification priors (KAS `input_record_classifications`, `record_sink` side channel, `record_priors` in `prepare_dk_classification_context`). The three `ResultItem` seams remain untouched (P3–P4); agentic gets no record priors yet.
