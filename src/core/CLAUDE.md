...
- ✅ **`BibRecord` (`src/core/bib_record.py`, WP-D1, July 20)** — one bibliographic record shape; `to_bibrecord(record, source)` normalizes finc/catalog/sru/k10plus. **First consumer landed (P1, Aug 3):** `to_analysis_text()` + the `isbn`/`ppn` input sources (`src/utils/input_sources/bib_lookup.py`); the three `ResultItem` seams remain untouched (P2–P4).
