...
- ✅ **Canonical GND-pool vocabulary (WP-D1 P0, July 19; verified July 20)**: one shape end-to-end — `{count, gnd_ids, classifications: {system: codes}, display_count?}` (suggester contract v2 → nested → pool → persisted KAS). System keys are **UPPERCASE** (`"DK"`/`"DDC"`/`"RVK"`, equal-rank) — `src/utils/classification_systems.py` is the single owner. Classic↔agentic convergence is gated by `tests/test_e2e_smoke.py::TestClassicAgenticPoolConvergence`. Spec: [`docs/wp_records_as_first_class.md`](../../docs/wp_records_as_first_class.md) "Pinned decisions".
