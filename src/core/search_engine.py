...
class SearchEngine(QObject):
    ...
    async def _make_network_request(self, request: QNetworkRequest) -> Optional[QNetworkReply]:
        """
    ...
    def process_results(self, current_results, threshold: float) -> Dict:
        ...
        exact_matches = []
        frequent_matches = []
        search_terms = [term.strip().lower() for term in self.current_results['term_results'].keys()]
