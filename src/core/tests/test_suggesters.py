...
class TestSuggesters(unittest.TestCase):
    ...
    def test_lobid_suggester(self):
        ...
        try:
            suggester = LobidSuggester(
                data_dir=self.test_data_dir / "lobid", debug=True
            )
            ...
            for result in unified_results:
                ...
                self.assertEqual(
                    len(result), 6
                )  # Keyword, GND ID, DDC, DK, count, search_term
    ...
    def test_swb_suggester(self):
        ...
        try:
            suggester = SWBSuggester(data_dir=self.test_data_dir / "swb", debug=True)
    ...
    def test_catalog_fallback_suggester(self):
        ...
        try:
            ...
            suggester = CatalogFallbackSuggester(
                data_dir=self.test_data_dir / "catalog_fallback", debug=True
            )
            ...
            )  # Just one term for faster testing
