...
MODE_SUCHE = (
    ...
    "  DK-Codes, GND-ID.\n"
    "- Katalog-Links: Wenn du Treffer aus `search_catalog_titles`\n"
    "  zitierst, verwende EXAKT dieses Marker-Format, damit der\n"
    "  Chat-Renderer einen klickbaren Link daraus bauen kann:\n"
    "    <<CAT:RSN|Anzeigetext>>\n"
    "  RSN ist die numerische RSN aus dem `rsn`-Feld des Treffers\n"
    "  (vom Tool geliefert). Anzeigetext = z.B. der Titel oder\n"
    "  '→ Katalog'. NIEMALS die URL selbst konstruieren\n"
    "  (https://katalog.ub.tu-freiberg.de/Record/…) — der Renderer\n"
    "  bildet sie zentral aus der RSN. Wenn kein `rsn` im Treffer,\n"
    "  nur den Anzeigetext ohne Marker schreiben.\n\n"
    "Was du NICHT tun sollst:\n"
