...
SHARED_RULES = (
    ...
    "- KEINE externen Links erfinden: Verwende ausschließlich URLs aus\n"
    "  Tool-Ergebnissen (web_url-Felder). Keine Google-Suchen, keine\n"
    "  Wikipedia, keine DOI.org, keine anderen Seiten, die nicht von\n"
    "  einem Tool zurückgegeben wurden. Wenn kein Tool eine URL\n"
    "  liefert → kein Link.\n\n"
...
MODE_SUCHE = (
    ...
    "  DK-Codes, GND-ID.\n"
    "- Katalog-Links (ZWINGEND — IMMER bei Buchtreffern angeben):\n"
    "  Wenn ein Treffer ein `web_url`-Feld enthält (`search_finc`,\n"
    "  `search_catalog_titles`), MUSS der Titel als Markdown-Link\n"
    "  ausgegeben werden:\n"
    "    [Titel des Buches](web_url)\n"
    "  Beispiel: [Tiefe Geothermie](https://katalog.ub.tu-freiberg.de/Record/0-123)\n"
    "  Jeder Treffer bekommt seinen eigenen Markdown-Link — keine\n"
    "  Treffer ohne Link auflisten, wenn `web_url` vorhanden.\n"
    "  Wenn kein `web_url`, aber numerische `rsn` vorhanden:\n"
    "    <<CAT:RSN|Titel>>\n"
    "  Wenn weder `web_url` noch `rsn` → nur den Titeltext.\n\n"
    "Was du NICHT tun sollst:\n"
...
MODE_GENERAL = (
    ...
    "- Katalog-Links (IMMER bei Buchtreffern): Wenn ein Treffer\n"
    "  ein `web_url`-Feld enthält, gib den Titel als Markdown-Link\n"
    "  aus: [Titel](web_url). Kein Link nur bei fehlendem `web_url`.\n\n"
    "Was du NICHT tun sollst:\n"
