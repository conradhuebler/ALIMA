...
- **One LLM pick per pipeline tab**: `pipeline_tab.global_override_selector` drives the run and, via `stream_widget.set_llm_override`, the chat. The chat header shows the effective model read-only (`model_status_label`) — ⚠️ do not add a second picker there; the old one silently outranked the toolbar's.
