...
class AbstractTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.results_list.setToolTip(
            "Klicken Sie auf einen Eintrag, um das Ergebnis anzuzeigen"
        )
        ...
        self.copy_button.setToolTip(
            "Kopiert das ausgewählte Ergebnis in die Zwischenablage"
        )
            ...
            if isinstance(subj, str):
                ...
                for separator in [",", ";", "\n", " - "]:
                    if separator in subj:
                        keywords.extend(
                            [k.strip() for k in subj.split(separator) if k.strip()]
                        )
                        break
                else:
                    # Falls kein Separator gefunden wurde
    ...
    def on_generation_finished(self, message):
        ...
        item = QListWidgetItem(
            f"{self.provider_combo.currentText()}: {self.model_combo.currentText().strip()}"
        )
