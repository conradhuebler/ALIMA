...
- **Knowledge tools**: Wrap `UnifiedKnowledgeManager` (search_gnd, get_gnd_entry, etc.)
  ...
  - `about_alima` → the project's own facts (name expansion, purpose, institution, publication with full citation + DOI, repository, licence) from `src/core/about.py`. Pure lookup: no config, no DB, so "what is ALIMA?" answers even when nothing else is reachable. The prompts forbid answering from memory, and an invented citation is the classic plausible-but-wrong failure. Handler: `ToolRegistry._handle_about_alima`.
